#!/usr/bin/env python3
"""
Focused test for file upload system in RAGnetic.
Tests both regular file uploads and temporary file uploads.
"""
...
class FileUploadTester:
    ...
    def create_test_user(self) -> Dict[str, Any]:
        ...
        user_data = {
            "username": f"file-test-user-{timestamp}",
            "email": f"filetest{timestamp}@example.com",
            "first_name": "File",
            "last_name": "Test",
            "scope": "editor",
    ...
    def create_api_key_for_user(self, user_id: int) -> str:
        ...
        key_data = {
            "name": f"file-test-key-{int(time.time())}",
            "scope": "editor"
        }
    ...
    def create_test_agent(self, user_api_key: str) -> Dict[str, Any]:
        ...
        timestamp = int(time.time())
        agent_config = {
            "name": f"file-test-agent-{timestamp}",
            "display_name": f"File Test Agent {timestamp}",
            "description": f"Test agent for file upload testing at {timestamp}",
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                self.log(f"[PASS] Created agent: {agent_info.get('agent', 'file-test-agent')}")
                return agent_info
            else:
...
"""
        ...
        files = {
            'file': ('test-document.md', test_content, 'text/markdown')
        }
    ...
    def create_chat_session(self, user_api_key: str, agent_name: str) -> str:
    ...
    def test_temporary_file_upload(self, user_api_key: str, thread_id: str) -> bool:
        ...
        # Create test content
        test_content = """# Temporary Test Document

This is a temporary document for chat testing.
...
"""
        ...
        files = {
            'file': ('temp-test-document.md', test_content, 'text/markdown')
        }
    ...
    def run_file_upload_test(self):
        """Run the complete file upload test."""
        self.log("[SUCCESS] Starting File Upload System Test")
        self.log("=" * 50)
        ...
        agent_name = agent.get('agent', 'file-test-agent')
        thread_id = self.create_chat_session(user_api_key, agent_name)
        if not thread_id:
        ...
        # Summary
        self.log("\n" + "=" * 50)
        self.log("[COMPLETE] File Upload Test Completed!")
...
def main():
    ...
    try:
        success = tester.run_file_upload_test()
        if success:
            print("\n[COMPLETE] File upload system test completed!")
            sys.exit(0)
        else:
            print("\n[FAIL] File upload system test failed.")
            sys.exit(1)
