"""Add parent_run_id to agent_runs and agent_run_steps tables

Revision ID: d906b26835c5
Revises: efeccc696266
Create Date: 2025-08-03 19:08:37.215291
...
revision: str = 'd906b26835c5'
down_revision: Union[str, Sequence[str], None] = 'efeccc696266'
...
def upgrade() -> None:
    ...
    with op.batch_alter_table('agent_run_steps', schema=None) as batch_op:
        batch_op.add_column(sa.Column('parent_run_id', sa.String(length=255), nullable=True))
        batch_op.create_index(batch_op.f('ix_agent_run_steps_parent_run_id'), ['parent_run_id'], unique=False)
        batch_op.create_foreign_key(batch_op.f('fk_agent_run_steps_parent_run_id_agent_runs'), 'agent_runs', ['parent_run_id'], ['run_id'])

    with op.batch_alter_table('agent_runs', schema=None) as batch_op:
        batch_op.add_column(sa.Column('parent_run_id', sa.String(length=255), nullable=True))
        batch_op.create_index(batch_op.f('ix_agent_runs_parent_run_id'), ['parent_run_id'], unique=False)
        batch_op.create_foreign_key(batch_op.f('fk_agent_runs_parent_run_id_agent_runs'), 'agent_runs', ['parent_run_id'], ['run_id'])
...
def downgrade() -> None:
    ...
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('agent_runs', schema=None) as batch_op:
        batch_op.drop_constraint(batch_op.f('fk_agent_runs_parent_run_id_agent_runs'), type_='foreignkey')
        batch_op.drop_index(batch_op.f('ix_agent_runs_parent_run_id'))
        batch_op.drop_column('parent_run_id')

    with op.batch_alter_table('agent_run_steps', schema=None) as batch_op:
        batch_op.drop_constraint(batch_op.f('fk_agent_run_steps_parent_run_id_agent_runs'), type_='foreignkey')
