...
def load(file_path: str) -> List[Document]:
    ...
    try:
        ...
        if not safe_file_path.exists():
            logger.error(f"Error: DOCX file not found at {safe_file_path}") # Changed from print()
            return []
        ...
        document = docx.Document(safe_file_path)
        ...
        if full_text.strip():
            ...
            logger.info(f"Loaded content from {safe_file_path.name}") # Changed from print()
            return [doc]
        else:
            logger.warning(f"DOCX file {safe_file_path.name} contained no readable text. Skipping document creation.")
            return []
    except Exception as e:
        logger.error(f"Error loading .docx file {file_path}: {e}", exc_info=True) # Changed from print() and added exc_info
        return []