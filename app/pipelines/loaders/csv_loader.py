...
def _apply_data_policies(text: str, policies: List[DataPolicy]) -> tuple[str, bool]:
    ...
    """
    processed_text = text
    document_blocked = False # In CSV context, this refers to blocking a row/document
            ...
            for keyword in kw_config.keywords:
                if keyword in processed_text:
                    if kw_config.action == 'redact':
                        ...
                        # At this stage (row/document level), we can't block just a chunk directly.
                        # We'll redact and log a warning for now, indicating this row contains content
                        # that should ideally be split and then blocked at chunking.
                        replacement = kw_config.redaction_placeholder or (kw_config.redaction_char * len(keyword))
                        processed_text = processed_text.replace(keyword, replacement)
                        logger.warning(f"Keyword '{keyword}' found. This row contains content that should ideally be blocked at chunk level. Currently redacting.")
                    elif kw_config.action == 'block_document': # In CSV loader, 'document' refers to a 'row'
                        logger.warning(f"Keyword '{keyword}' found. Row is marked for blocking by policy. Content will be discarded.")
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            if agent_config and agent_config.data_policies:
                logger.debug(f"Applying data policies to row {index + 1} of {safe_file_path.name}...")
                processed_text, document_blocked = _apply_data_policies(page_content, agent_config.data_policies)

            if document_blocked:
                logger.warning(f"Row {index + 1} of '{safe_file_path.name}' was completely blocked by a data policy and will not be processed.")
            ...
            if processed_text.strip(): # Ensure there's actual content left after policies
                ...
                logger.debug(f"Row {index + 1} of '{safe_file_path.name}' had no content after policy application or was empty.")
