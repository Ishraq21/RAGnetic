...
from app.pipelines.loaders import directory_loader, url_loader, pdf_loader, docx_loader, csv_loader, code_loader, \
    db_loader, gdoc_loader, web_crawler_loader, api_loader
...
def load_documents_from_source(source: dict) -> list[Document]:
    ...
    # (Keep the existing loader logic, it is correct)
    if source_type == "local":
        ...
        return gdoc_loader.load(folder_id=source.get("folder_id"), document_ids=source.get("document_ids"),
                                file_types=source.get("file_types"))
    elif source_type == "web_crawler":
        return web_crawler_loader.load(url=source.get("url"), max_depth=source.get("max_depth", 2))
    elif source_type == "api":
        return api_loader.load(url=source.get("url"), method=source.get("method", "GET"), headers=source.get("headers"),
                               params=source.get("params"), payload=source.get("payload"),
...
def embed_agent_data(config: AgentConfig, openai_api_key: str = None):
    ...
    for source in config.sources:
        loaded_docs = load_documents_from_source(source.model_dump())
    ...
    # The get_api_key function is already called in the CLI
