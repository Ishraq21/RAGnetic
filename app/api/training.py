...
from app.db.models import fine_tuned_models_table, billing_sessions_table
...
@router.post("/apply", response_model=FineTunedModel, status_code=status.HTTP_202_ACCEPTED, dependencies=[Depends(rate_limit_dep("training_apply", 3, 60))])
...
@router.post("/upload-dataset", dependencies=[Depends(rate_limit_dep("training_upload", 10, 60))])
...
@router.post("/jobs/gpu", response_model=FineTunedModel, status_code=status.HTTP_202_ACCEPTED, dependencies=[Depends(rate_limit_dep("gpu_training", 2, 60))])
async def create_gpu_training_job(
    job_config: FineTuningJobConfig,
    current_user: User = Depends(PermissionChecker(["fine_tune:initiate"])),
    db: AsyncSession = Depends(get_db),
):
    """
    Create a GPU-based fine-tuning job with automatic provisioning and cost estimation.
    """
    logger.info(f"API: User '{current_user.username}' submitted GPU fine-tuning job '{job_config.job_name}'.")
    
    # Validate GPU configuration
    if not job_config.use_gpu:
        raise HTTPException(
            status_code=400,
            detail="use_gpu must be True for GPU training jobs"
        )
    
    if not job_config.gpu_type:
        raise HTTPException(
            status_code=400,
            detail="gpu_type is required for GPU training jobs"
        )
    
    if not job_config.max_hours:
        raise HTTPException(
            status_code=400,
            detail="max_hours is required for GPU training jobs"
        )
    
    # Validate dataset path (reuse existing allowlist logic)
    from app.core.config import get_user_training_data_paths
    user_paths = get_user_training_data_paths(current_user.id)
    allowed_roots = [
        Path(os.getenv("RAGNETIC_DATASET_ROOT", "data")).resolve(),
    ...
    try:
        ...
        gpu_orchestrator = GPUOrchestrator(db)
        
        # Estimate cost
        estimated_cost = await cost_service.estimate_training_cost(
            job_config.gpu_type, 
            job_config.max_hours, 
            job_config.gpu_provider or "runpod"
        )
        
        # Check credits and limits
        await credit_service.ensure_balance(current_user.id, estimated_cost)
        
        if not await credit_service.within_limits(current_user.id, estimated_cost):
            raise HTTPException(
                status_code=400,
                detail=f"Request exceeds daily spending limit. Estimated cost: ${estimated_cost:.2f}"
            )
        
        # Create billing session
        billing_result = await db.execute(
            insert(billing_sessions_table).values(
                user_id=current_user.id,
                session_type="training",
                total_cost=estimated_cost,
                started_at=datetime.utcnow()
            ).returning(billing_sessions_table)
        )
        billing_session = billing_result.fetchone()
        
        # Provision GPU
        job_config_dict = {
            "gpu_type": job_config.gpu_type,
            "max_hours": job_config.max_hours,
            "purpose": "training",
            "image": "ragnetic/training:latest",
            "request_data": {
        ...
        gpu_instance_id = await gpu_orchestrator.provision_for_training(
            job_config_dict, 
            current_user.id
        )
        ...
        if not gpu_instance_id:
            raise HTTPException(
                status_code=500,
                detail="Failed to provision GPU instance"
            )
        ...
        # Update job config with GPU instance ID
        job_cfg = job_config.model_dump()
        job_cfg["adapter_id"] = generated_adapter_id
        job_cfg["gpu_instance_id"] = str(gpu_instance_id)
        ...
        logger.info(f"API: GPU fine-tuning job '{job_config.job_name}' with ID '{generated_adapter_id}' dispatched.")
        return FineTunedModel.model_validate(created_job_row)
        ...
        await db.rollback()
        logger.error(f"API: Failed to submit GPU fine-tuning job '{job_config.job_name}': {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to submit GPU fine-tuning job: {e}")


@router.get("/jobs/{adapter_id}/logs")
async def get_training_logs(
    adapter_id: str,
    tail_lines: int = Query(100, ge=1, le=1000, description="Number of lines to retrieve from the end"),
    current_user: User = Depends(PermissionChecker(["fine_tune:read_status"])),
    db: AsyncSession = Depends(get_db),
):
    """
    Get training logs for a specific job, either from local logs or GPU provider logs.
    """
    try:
        # Get job details
        stmt = select(fine_tuned_models_table).where(
        ...
        if not job_data:
            raise HTTPException(status_code=404, detail=f"Training job '{adapter_id}' not found.")
        
        # Check if it's a GPU job
        if job_data.get("gpu_instance_id"):
            # Get logs from GPU provider
            gpu_orchestrator = GPUOrchestrator(db)
            logs = await gpu_orchestrator.get_instance_logs(
                int(job_data["gpu_instance_id"]), 
                current_user.id,
                tail_kb=tail_lines // 10  # Rough conversion from lines to KB
            )
            
            if logs:
                return {"logs": logs, "source": "gpu_provider"}
        ...
        if logs_path and Path(logs_path).exists():
            try:
                with open(logs_path, 'r') as f:
                    ...
                    recent_lines = all_lines[-tail_lines:] if len(all_lines) > tail_lines else all_lines
                    logs_content = ''.join(recent_lines)
                    return {"logs": logs_content, "source": "local_file"}
            except Exception as e:
        ...
        logger.error(f"Failed to get training logs for '{adapter_id}': {e}")
        raise HTTPException(status_code=500, detail=f"Failed to retrieve training logs: {e}")