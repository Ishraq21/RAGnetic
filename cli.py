...
def init():
    ...
    paths_to_create = {
        ...
        "LOGS_DIR", "TEMP_CLONES_DIR", "RAGNETIC_DIR", "BENCHMARK_DIR", "TRAINING_CONFIGS_DIR", "DATA_RAW_DIR", "FINE_TUNED_MODELS_BASE_DIR", "DATA_PREPARED_DIR",
        "DATA_PREP_CONFIGS"
    }
...
def inspect_run(
    ...
    try:
        with engine.connect() as connection:
            ...
            if not run:
                typer.secho(f"Error: Run with ID '{run_id}' not found in agent tables.",
                            fg=typer.colors.RED)
                raise typer.Exit(code=1)
